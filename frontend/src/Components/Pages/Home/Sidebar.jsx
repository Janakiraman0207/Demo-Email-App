...
const labelItems = ["Colleague", "Friends", "Office", "Others"];
...
export const Sidebar = () => {
  ...
  const handleMailboxClick = async (itemKey) => {
    try {
      const apiCall = MAILBOX_API_MAP[itemKey];

      if (!apiCall) {
        console.error("No API mapped for:", itemKey);
        return;
      }
  ...
  return (
    <div className="flex flex-col w-[158px] h-full bg-[#040B23] overflow-hidden">
          ...
          <div className="flex flex-col h-[200px] gap-[10px] py-[10px]">
            {items.map((item, index) => (
              <div
                ...
                className={`flex items-center justify-between w-[158px] ${selectedMailboxIndex === index ? "h-[44px]" : ""} py-[1px] pr-[16px] pl-[28px] gap-[px] cursor-pointer ${selectedMailboxIndex === index ? "bg-[#6A37F5]" : ""}`}
              >
                <span
                  className={`${selectedMailboxIndex === index ? "inter-bold text-[12px] tracking-[0.07em] leading-[100%]" : "inter-regular text-[11px]"} ${selectedMailboxIndex === index ? "text-[#FFFFFF]" : "text-[#B6B6B6]"} hover:text-[#FFFFFF]`}
                >
                  {item.label}
                </span>
                {index === 0 && (
          ...
          <div className="flex flex-col h-[200px] gap-[10px] py-[10px]">
            {othersItems.map((item, index) => (
              <div
                key={index}
                onClick={() => {
                  setSelectedOthersIndex(index);
                  setSelectedMailboxIndex(null);
                  setSelectedLabelIndex(null);
                  handleMailboxClick(item.key);
                }}
                className={`flex items-center w-[158px] ${selectedOthersIndex === index ? "h-[44px]" : ""} py-[1px] pr-[16px] pl-[28px] gap-[px] cursor-pointer ${selectedOthersIndex === index ? "bg-[#6A37F5]" : ""}`}
              >
                <span
                  className={`${selectedOthersIndex === index ? "inter-bold text-[12px] tracking-[0.07em] leading-[100%]" : "inter-regular text-[11px]"} ${selectedOthersIndex === index ? "text-[#FFFFFF]" : "text-[#B6B6B6]"} hover:text-[#FFFFFF]`}
                >
                  {item.label}
                </span>
              </div>
